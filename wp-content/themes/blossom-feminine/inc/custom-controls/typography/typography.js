 ...
 */
wp.customize.controlConstructor['blossom-feminine-typography'] = wp.customize.Control.extend( {
    ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( blossom_all_fonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.variant-wrapper' ).css( 'display', 'block' );
				}
